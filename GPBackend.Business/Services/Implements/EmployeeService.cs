...
namespace GPBackend.Services
{
    public class EmployeeService : IEmployeeService
    {
        ...
        public async Task<EmployeeDto> GetEmployeeByIdAsync(int id, int userId)
        {
            ...
            if (employee == null)
            {
                throw new GPBackend.Exceptions.NotFoundException($"Employee with Id {id} not found");
            }
        ...
        public async Task<int> BulkDeleteEmployeesAsync(IEnumerable<int> ids, int userId)
        {
            return await _employeeRepository.BulkSoftDeleteAsync(ids, userId);
        }
    }
} 
